...
import IrisaTextField from '../../itcomponents/TextField/IrisaTextField'
...
export default function Form1(props){
    ...
    const TestTextField=()=>( 
        ...
        <br />
        <IrisaTextField name={"txt3"} value={valarray.txt3.value}  variant={state.select}
        rules={{trigger: 'blur' ,validationType:["lowercase"],validationParams:[100],onCustomValidation:true}}/>
    
        <br />
        <IrisaTextField name={"txt4"} value={valarray.txt4.value}  variant={state.select} 
        type={"mask"}
        mask={['(', /[1-9]/, /\d/, /\d/, ')', ' ', /\d/, /\d/, /\d/, '-', /\d/, /\d/, /\d/, /\d/]}    
        />
        <br />
        <IrisaTextField name={"txt5"} value={valarray.txt4.value}  variant={state.select}
        type={"number"}
          prefix={"ریال"}/>
        <br /> 
        </>)
        ...
        <div>
            ...
            <FormControl>
              <InputLabel>Variant</InputLabel>
                <Select label={"label1"} name={"select"} value={state.select} onChange={handlerStateChange} >
                    <MenuItem value={"standard"} label={"standard"} >standard</MenuItem>
                    <MenuItem value={"outlined"} label={"outlined"} >outlined</MenuItem>
