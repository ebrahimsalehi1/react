...
var createCompare = function (sorting, getColumnCompare, getComparableValue) { return sorting.slice()
    .reverse()
    .reduce(function (prevCompare, columnSorting) {
...
var groupedRows = function (rows, grouping, getCellValue, getColumnCriteria, keyPrefix) {
    ...
    return __spread(groups.values()).reduce(function (acc, _a) {
        var _b = __read(_a, 3), value = _b[0], key = _b[1], items = _b[2];
        var _c;
        ...
        acc.push((_c = {
            ...
            _c[GRID_GROUP_CHECK] = true,
            _c[GRID_GROUP_LEVEL_KEY] = GRID_GROUP_TYPE.toString() + "_" + groupedBy,
            _c));
...
var customGroupedRows = function (currentRows, grouping, getChildGroups, rootRows, keyPrefix) {
    ...
    return getChildGroups(currentRows, grouping[0], rootRows)
        ...
        var key = _a.key, _b = _a.value, value = _b === void 0 ? key : _b, childRows = _a.childRows;
        var _c;
        var compoundKey = "" + keyPrefix + key;
        acc.push((_c = {
                groupedBy: groupedBy,
...
var tableRowsWithEditing = function (tableRows, editingRowIds, addedRows, rowHeight) {
    ...
    return __spread(addedTableRows.reverse(), editedTableRows);
};
...
var tableRowsWithDataRows = function (rows, getRowId) { return (!rows.length
...
var expandedTreeRows = function (_a, getRowId, expandedRowIds) {
    ...
    return rows.reduce(function (acc, row) {
        ...
        acc.rows.push(row);
        return acc;
    }, { treeMeta: treeMeta, rows: [], collapsedRowsMeta: new Map() });
};
...
var getVisibleBoundary = function (items, viewportStart, viewportSize, getItemSize, overscan) {
    var start = null;
    var end = null;
...
var getCollapsedRows = function (rows, visibleBoundary, boundaries, getRowHeight, getCells) {
    var collapsedRows = [];
    boundaries.forEach(function (boundary) {
...
var getCollapsedGrid = function (_a) {
    var rows = _a.rows, columns = _a.columns, top = _a.top, height = _a.height, left = _a.left, width = _a.width, _b = _a.getColumnWidth, getColumnWidth = _b === void 0 ? (function (column) { return column.width; }) : _b, _c = _a.getRowHeight, getRowHeight = _c === void 0 ? (function (row) { return row.height; }) : _c, _d = _a.getColSpan, getColSpan = _d === void 0 ? function () {
        var args = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            args[_i] = arguments[_i];
        }
    ...
    return {
        ...
        rows: getCollapsedRows(rows, rowsVisibleBoundary, rowBoundaries, getRowHeight, function (row) { return getCollapsedCells(columns, getSpanBoundary(columns, columnsVisibleBoundary, function (column) { return getColSpan(row, column); }), columnBoundaries, function (column) { return getColSpan(row, column); }); }),
    };
};
